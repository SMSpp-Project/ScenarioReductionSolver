...
ScenarioReductionState::ScenarioReductionState()
{
 ...
 config->set_par( std::string( "strReductionMethod" ) ,
                  std::string( "dupacova" ) );
 ...
 config->set_par( std::string( "strSolverConfig" ) ,
...
void print_help( const char * program_name )
{
 ...
 cout << "  -v, --verbose <level>      Set verbosity level (0=silent,"
         " 1=normal, 2=detailed)" << endl;
 cout << "  -t, --time <seconds>       Set solver time limit in seconds"
      << endl;
 cout << "  -c, --solver-config <file> Solver config file (default:"
         " BSPar_HiGHS.txt)" << endl;
 ...
 cout << "                             baseline, dupacova (default), bestfit,"
         " firstfit, milp, cssc" << endl;
 ...
 cout << "  --skip-full                Skip solving the full N-scenario"
         " problem" << endl;
                      ...
                      int argc , char * argv[] )
{
 ...
 for( int i = 1 ; i < argc ; ++i ) {
  ...
  if( arg == "-h" || arg == "--help" ) {
   ...
   if( i + 1 < argc )
    config->set_par( std::string( "strInstancePath" ) ,
                     std::string( argv[ ++i ] ) );
   else {
   ...
   if( i + 1 < argc )
    config->set_par( std::string( "strScenarioFile" ) ,
                     std::string( argv[ ++i ] ) );
   else {
    cerr << "Missing value after " << arg << endl;
    exit( 1 );
   ...
   if( i + 1 < argc ) {
    ...
    if( method != "baseline" && method != "dupacova" && method != "bestfit" &&
     ...
     cerr << "Valid methods: baseline, dupacova, bestfit, firstfit,"
             " milp, cssc" << endl;
     exit( 1 );
         }
    config->set_par( std::string( "strReductionMethod" ) ,
...
void print_configuration( ScenarioReductionState & state )
{
 ...
 if( get_int_config( config , "intLogVerb" ) >= 1 ) {
  ...
  cout << "  Instance.........: "
       << get_str_config( config , "strInstancePath" ) << endl;
  if( ! get_str_config( config , "strScenarioFile" ).empty() )
   cout << "  Scenario file...: "
        << get_str_config( config , "strScenarioFile" ) << endl;
  if( get_int_config( config , "intFullNumScenarios" ) > 0 )
   cout << "  Full scenarios..: "
        << get_int_config( config , "intFullNumScenarios" ) << endl;
  cout << "  Reduced scenarios: "
       << get_int_config( config , "intReducedNumScenarios" ) << endl;
  cout << "  Reduction method.: "
       << get_str_config( config , "strReductionMethod" ) << endl;
  cout << "  Verbose level....: "
       << get_int_config( config , "intLogVerb" ) << endl;
  if( get_dbl_config( config , "dblMaxTime" ) > 0 )
   cout << "  Time limit......: "
        << get_dbl_config( config , "dblMaxTime" ) << " seconds" << endl;
  if( get_int_config( config , "intSaveResults" ) ||
      get_int_config( config , "intLoadResults" ) ) {
   cout << "  Cache directory.: "
        << get_str_config( config , "strCacheDir" ) << endl;
   if( get_int_config( config , "intSaveResults" ) )
  ...
  if( get_int_config( config , "intSkipFull" ) )
   cout << "  Skip full solve: enabled" << endl;
                               ...
                               const std::string & instance_path )
{
 ...
 return( get_scenarios_directory( hooks ) + p.stem().string() +
         "_scenarios.nc4" );
 }
...
{
 auto * config = state.config;
 string inst   = extract_instance_name(
  state , get_str_config( config , "strInstancePath" ) );
 ...
 if( get_int_config( config , "intLogVerb" ) >= 2 )
  cout << "[DEBUG] Generated cache filename: " << result
       << " (full=" << is_full << ", anticipative=" << is_anticipative
       << ")" << endl;
 return( result );
 }
...
void save_solutions_cache( ScenarioReductionState & state )
{
 ...
 if( state.full_result.solved ) {
  ...
  if( get_int_config( state.config , "intLogVerb" ) >= 2 )
   cout << "[DEBUG] Saved full extensive: " << f << endl;
  }
 if( state.reduced_result.solved ) {
  ...
  if( get_int_config( state.config , "intLogVerb" ) >= 2 )
   cout << "[DEBUG] Saved reduced extensive: " << f << endl;
  }
 if( state.anticipative_full.solved ) {
  ...
  if( get_int_config( state.config , "intLogVerb" ) >= 2 )
   cout << "[DEBUG] Saved anticipative full: " << f << endl;
  }
 if( state.anticipative_reduced.solved ) {
  ...
  if( get_int_config( state.config , "intLogVerb" ) >= 2 )
   cout << "[DEBUG] Saved anticipative reduced: " << f << endl;
  }
                             ...
                             const ScenarioReductionMetrics & metrics )
{
 ...
 try {
  ...
  if( fp.has_parent_path() ) filesystem::create_directories( fp.parent_path() );
  netCDF::NcFile file( filename , netCDF::NcFile::replace );
  file.putAtt( "reduction_time_ms" , netCDF::NcInt64() ,
               metrics.reduction_time_ms );
  file.putAtt( "ell"                  , netCDF::NcDouble() , metrics.ell );
  file.putAtt( "wasserstein_distance" , netCDF::NcDouble() ,
               metrics.wasserstein_distance );
  file.putAtt( "wasserstein_ell_power" , netCDF::NcDouble() ,
               metrics.wasserstein_ell_power );
  ...
  cerr << "Warning: Failed to save reduction solution cache: "
       << e.what() << endl;
  }
 }
...
ScenarioReductionMetrics load_reduction_solution(
 ScenarioReductionState & state , const string & filename )
{
 if( get_int_config( state.config , "intLogVerb" ) >= 2 )
  cout << "[DEBUG] Loading reduction solution from cache: " << filename << endl;
 if( ! filesystem::exists( filename ) )
  throw( runtime_error(
   "load_reduction_solution: cache file not found: " + filename ) );
 ...
 file.getAtt( "wasserstein_distance" )
     .getValues( & metrics.wasserstein_distance );
 file.getAtt( "wasserstein_ell_power" )
     .getValues( & metrics.wasserstein_ell_power );
 try {
                             ...
                             const std::vector< double > & scenario )
{
 if( ! state.stochastic_block )
  throw( runtime_error(
   "apply_scenario_to_block: StochasticBlock not initialized, ensure "
   "the TwoStageStochasticBlock is created first" ) );
...
pair< double , bool > solve_block( ScenarioReductionState & state ,
                                   Block * block )
{
 try {
  auto cfg = Configuration::deserialize(
   get_str_config( state.config , "strSolverConfig" ) );
  ...
  if( ! block->get_registered_solvers().empty() ) {
   ...
   if( get_dbl_config( state.config , "dblMaxTime" ) > 0 )
    solver->set_par( Solver::dblMaxTime ,
                     get_dbl_config( state.config , "dblMaxTime" ) );
                                      ...
                                      TwoStageStochasticBlock * tss_block )
{
 ...
 try {
  ...
  if( ! bsc )
   throw( runtime_error(
    "compute_extensive_form: failed to load solver configuration" ) );
  bsc->apply( tss_block );
...
SolutionResult solve_anticipative_solution( ScenarioReductionState & state )
{
 ...
 for( size_t i = 0 ; i < selected_indices.size() ; ++i ) {
  ...
  for( size_t j = 0 ; j < scenario_size ; ++j )
   scenario[ j ] = state.scenario_set->get_scenario_value(
...
void load( ScenarioReductionState & state , ProblemHooks & hooks )
{
 ...
 if( ! state.base_block )
  throw( runtime_error(
   "load: failed to load base block from " +
   get_str_config( config , "strInstancePath" ) ) );
 if( ! state.stochastic_block )
  throw( runtime_error(
   "load: failed to create stochastic block for " +
   get_str_config( config , "strInstancePath" ) ) );
 ...
 if( ! get_str_config( config , "strScenarioFile" ).empty() ) {
  ...
  scenario_file = get_scenario_file(
   hooks , get_str_config( config , "strInstancePath" ) );
  if( get_int_config( config , "intLogVerb" ) >= 2 )
 ...
 try {
  ...
  throw( runtime_error(
   "load: failed to load scenarios from: " + scenario_file + "\n\n"
...
void solve_scenario_reduction( ScenarioReductionState & state ,
                               ProblemHooks & hooks )
{
 auto * config = state.config;
  ...
  try {
   ...
   if( ! state.reduction_metrics.selected_indices.empty() ) {
    if( get_int_config( config , "intLogVerb" ) >= 1 ) {
     ...
     cout << "  Note: Will need to recompute reduction for exact indices"
          << endl;
     }
    }
   }
 ...
 for( int p = 0 ; p < Ks ; ++p ) {
  state.reduction_metrics.selected_indices.push_back(
...
void solve_stochastic_problem( ScenarioReductionState & state ,
                               ProblemHooks & hooks )
{
 auto * config = state.config;
 if( get_int_config( config , "intLogVerb" ) >= 1 )
 ...
 if( get_int_config( config , "intLoadResults" ) ) {
  try {
   ...
   if( is_full )
    state.full_result    = load_solution_cache( state , cache_file );
   else
    state.reduced_result = load_solution_cache( state , cache_file );
   if( get_int_config( config , "intLogVerb" ) >= 1 )
...
void solve_anticipative( ScenarioReductionState & state )
{
 ...
 if( get_int_config( config , "intLoadResults" ) ) {
  ...
  try {
   ...
   loaded_reduced = true;
   if( get_int_config( config , "intLogVerb" ) >= 2 )
    cout << "[DEBUG] Successfully loaded anticipative reduced from cache"
         << endl;
   }
  catch( const exception & e ) {
   if( get_int_config( config , "intLogVerb" ) >= 2 )
  ...
  if( ( loaded_full || loaded_reduced )
      && get_int_config( config , "intLogVerb" ) >= 1 )
   ...
   cout << "  Could not load cached anticipative solutions, computing..."
        << endl;
  }
 ...
 bool is_reduced_pool = ( current_pool_size ==
  static_cast< size_t >(
   get_int_config( config , "intReducedNumScenarios" ) ) );
...
void print_results( ScenarioReductionState & state )
{
 ...
 if( state.reduced_result.solved )
  cout << "  Reduced solve t: "
       << state.reduced_result.time_ms << " us" << endl;

 cout << "\nObjectives" << endl;
 if( state.full_result.solved )
  cout << "  Full...: "
       << fixed << setprecision( 2 ) << state.full_result.objective << endl;
 else
  cout << "  Full...: NOT SOLVED" << endl;
 if( state.reduced_result.solved )
  cout << "  Reduced: " << fixed << setprecision( 2 )
       << state.reduced_result.objective << endl;
 else
  cout << "  Reduced: NOT SOLVED" << endl;

 if( state.full_result.solved && state.reduced_result.solved ) {
  double diff = abs( state.reduced_result.objective -
                     state.full_result.objective );
 ...
 if( get_int_config( config , "intComputeVPI" ) &&
  ...
  if( state.anticipative_reduced.solved && state.reduced_result.solved ) {
   double vpi = state.reduced_result.objective -
                state.anticipative_reduced.objective;
